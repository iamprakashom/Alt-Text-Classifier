    ...
    def filepatternFinder(altText, imgReso_value, fileextension=None):
        if fileextension == 'JPG':
            ...
            if has_image_num.match(altText):
                ...
                return csvfileWriter(altText, fileExit=1, fileExt_bmp=1,
                                     has_img_sym_num=1, imgResolution=imgReso_value)
        else:
            return csvfileWriter(altText, fileExt=1, imgResolution=imgReso_value)

    def alttextChecker(altText, imgReso_value=0):
        if (len(altText) == 0):
            csvfileWriter(altText, nullVal=1, imgResolution=imgReso_value)
        elif symbolOnlypattern.match(altText):
            csvfileWriter(altText, symbolOnly=1, imgResolution=imgReso_value)
        elif numOnlypattern.match(altText):
            csvfileWriter(altText, numOnly=1, imgResolution=imgReso_value)
        elif len(altText) <= 2:
            csvfileWriter(altText, stopWord=1, imgResolution=imgReso_value)
            ...
            if wordnet.synsets(altText):
                csvfileWriter(altText, textinDictionary=1, imgResolution=imgReso_value)
            else:
            ...
            if altText.lower().endswith('jpg') or altText.lower().endswith('jpeg'):
                ...
                csvfileWriter(altText, imgReso_value, fileExt=1, fileExt_pdf=1)
            ...
            if altText.lower().startswith('dsc'):
                ...
                csvfileWriter(altText, fileNamepattern=1, has_image_num=1, imgResolution=imgReso_value)
            elif has_image_sym_num.match(altText):
                csvfileWriter(altText, fileNamepattern=1, has_image_sym_num=1, imgResolution=imgReso_value)
            elif has_img_num.match(altText):
                csvfileWriter(altText, fileNamepattern=1, has_img_num=1, imgResolution=imgReso_value)
            elif has_img_sym_num.match(altText):
                csvfileWriter(altText, fileNamepattern=1, has_img_sym_num=1, imgResolution=imgReso_value)
            ...
            if wordandWordpattern.search(altText):
                if wordNumPercentageWordpattern.search(altText):
                    if wordDashWordpattern.search(altText):
                        if wordUnderscoreWordpattern.search(altText):
                            if wordSymbolwordpattern.search(altText):
                                csvfileWriter(altText, words=1, wordNumPercentageWord=1,
                    ...
                    if wordUnderscoreWordpattern.search(altText):
                        if wordSymbolwordpattern.search(altText):
                            csvfileWriter(altText, words=1, wordandWord=1,
                    ...
                    csvfileWriter(altText, words=1, wordandWord=1, imgResolution=imgReso_value)
                ...
                if wordDashWordpattern.search(altText):
                    ...
                    if wordSymbolwordpattern.search(altText):
                        csvfileWriter(altText, words=1, wordNumPercentageWordpattern=1,
                ...
                if wordUnderscoreWordpattern.search(altText):
                    if wordSymbolwordpattern.search(altText):
                        csvfileWriter(altText, words=1, wordDashWord=1,
                                      wordUnderscoreWord=1, wordSymbolword=1)
                    else:
                        csvfileWriter(altText, words=1, wordDashWord=1,
                                      wordUnderscoreWord=1)
                ...
                if wordSymbolwordpattern.search(altText):
                    csvfileWriter(altText, words=1, wordUnderscoreWord=1,
                                  wordSymbolword=1)
                else:
                    csvfileWriter(altText, words=1, wordUnderscoreWord=1)
            elif wordSymbolwordpattern.search(altText):
                csvfileWriter(altText, words=1, wordSymbolword=1)
            else:
                csvfileWriter(altText, words=1)
